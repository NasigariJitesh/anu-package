...
const selectLabelColorBasedOnState = (
	props: TextInputLabelProps,
	theme: DripsyFinalTheme,
) => {
	if (props.disabled)
		return props.color ?? getColorInRGBA(theme.colors.$onSurface, 38);
...
const selectLabelBackgroundColor = (
	props: TextInputLabelProps,
	theme: DripsyFinalTheme,
) => {
...
const TextFieldLabel = (props: TextInputLabelProps) => {
	...
	const textStyles = {
		...
		maxWidth: '100%',
		color:
			props.placeholderTextColor || selectLabelColorBasedOnState(props, theme),
	...
	const animatedStyle = {
		...
		paddingHorizontal:
			props.isFocused || props.value
				? 16
				: customTextStyle?.paddingHorizontal ?? 16,
	...
	const transitionIn = () => {
		setTimeout(() => {
			transitionTopCoordinate.value = withTiming(
				props.variant === 'outlined' ? -1 : 4,
				{
					duration: DURATION,
				},
			);
			transitionLeftCoordinate.value = withTiming(
				props.variant === 'outlined' ? 0 - props.leadingIconWidth : 0,
				{
					duration: DURATION,
				},
			);
	...
	const transitionOut = () => {
		...
		setTimeout(() => {
			...
			transitionLetterSpacing.value = withTiming(
