...
import {
	FileDropZoneReferenceProps,
	NativeFileDropZoneProps,
} from '../../types';
import {
	compressFileNative,
	getDropZoneStyles,
...
>((props, reference) => {
	...
	const { dropZoneStyle, childrenContainerStyle, buttonContainerStyle } =
		getDropZoneStyles(theme);
				...
				<Container disableGutters>
					...
					{finalProps.hideActionButtons ? null : (
						<Container disableGutters style={buttonContainerStyle}>
							<Button.Text
								title={finalProps.submitLabel || ''}
								onPress={finalProps.onSubmit}
							/>
							<Button.Text
								title={finalProps.cancelLabel || ''}
								onPress={onCancel}
							/>
						</Container>
					)}
