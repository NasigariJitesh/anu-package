	...
	if (result.type === 'success' && result.output) {
		if (props.uploadVariant === 'image' && props.optimization) {
			...
			for (const file of result.output) {
				const compressedImage = await compressFile(
					file,
 ...
 */
const FileUpload = forwardRef<FileUploadReferenceProps, FileUploadProps>(
	(props, reference) => {
		//@ts-expect-error
			...
			if (finalProps.multiple) {
				...
				if (finalProps.sortable) {
					for (const file of resultFiles) {
						...
						const similarNameFilesInSameUpload = uniqueResultFiles.filter(
							(item) => item.name === file.name,
						);

						if (
							similarNameFiles.length > 0 ||
							similarNameFilesInSameUpload.length > 0
						) {
				...
				if (finalProps.onChange && resultFiles[0])
					finalProps.onChange(resultFiles[0]);
			}
		};
		...
		const deleteFile = (index: number) => {
			...
			if (finalProps.onChange)
				finalProps.onChange(array.length > 0 ? array : null);
		};
