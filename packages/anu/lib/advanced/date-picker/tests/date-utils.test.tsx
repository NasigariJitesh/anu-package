...
describe('timeUtils', () => {
  ...
  it('should return correct unix time stamp for date', () => {
    expect(dateToUnix(new Date(2022, 1, 1))).toBe(1_643_653_800);
  });
  ...
  it('should indicate days are the same', () => {
    expect(areDatesOnSameDay(new Date(2022, 1, 1), new Date(2022, 1, 1))).toBeTruthy();
  });
  ...
  it('should return correct gridCount for October 2021', () => {
    expect(getGridCountForDate(addMonths(new Date(2018, 10 - 1, 1), 12 * 3))).toBe(6);
  });

  it('should get correct difference in month between dates', () => {
    expect(differenceInMonths(new Date(2022, 1, 1), new Date(2022, 2, 1))).toBe(1);
  });
});
